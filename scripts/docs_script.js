...
marked.setOptions({
    highlight: function(code, lang) {
        if (lang === 'luma' || lang === 'lx') {
            return Prism.highlight(code, Prism.languages.luma, 'luma');
        }
...
function generateTOC() {
    ...
    if (currentH1) {
        ...
        // No h1 headings found; render all h2s as top-level entries
        h2List.forEach(h2 => {
            tocHTML += `<li class="toc-h2"><a href="#${h2.id}" data-level="h2">${h2.text}</a></li>`;
        });
    }
